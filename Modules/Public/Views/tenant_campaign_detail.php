...
function shareCampaign() {
    ...
    const title = <?= json_encode($campaign['title'] ?? 'Urunan', JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
    const text = <?= json_encode(strip_tags($campaign['description'] ?? ''), JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
...
function showShareModal(url, title) {
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            ...
            <div class="space-y-3">
                <button onclick="shareToWhatsApp('${safeUrl}', '${safeTitle}')" class="w-full flex items-center gap-3 px-4 py-3 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-colors">
                ...
                <button onclick="shareToFacebook('${safeUrl}')" class="w-full flex items-center gap-3 px-4 py-3 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors">
                ...
                <button onclick="shareToTwitter('${safeUrl}', '${safeTitle}')" class="w-full flex items-center gap-3 px-4 py-3 bg-sky-50 hover:bg-sky-100 rounded-lg transition-colors">
                ...
                <button onclick="copyToClipboard('${safeUrl}')" class="w-full flex items-center gap-3 px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors">
...
function shareToWhatsApp(url, title) {
    ...
    const text = encodeURIComponent(title + ' - ' + decodedUrl);
...
function shareToTwitter(url, title) {
    ...
    window.open('https://twitter.com/intent/tweet?text=' + text + '&url=' + encodeURIComponent(decodedUrl), '_blank', 'width=600,height=400');
    document.querySelector('.fixed').remove();
}
